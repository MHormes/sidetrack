...
export default function BurgerMenu() {
  ...
  return (
    <>
      ...
      >
        <span
          className={`block h-0.5 w-6 bg-fg-muted transition-all duration-200 ${open ? "rotate-45 translate-y-2" : ""}`}
        />
        <span
          className={`block h-0.5 w-6 bg-fg-muted transition-all duration-200 ${open ? "opacity-0" : ""}`}
        />
        <span
          className={`block h-0.5 w-6 bg-fg-muted transition-all duration-200 ${open ? "-rotate-45 -translate-y-2" : ""}`}
        />
        ...
        <nav className="sm:hidden border-t border-edge bg-base px-4 py-4 flex flex-col gap-4 text-sm font-medium text-fg-muted absolute top-full left-0 right-0 z-50">
          {links.map(({ href, label, accent }) => (
            <Link
              ...
              className={
                accent
                  ? "text-accent hover:text-accent transition-colors"
